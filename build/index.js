...
const QuoteAPIEditorInspectorControls = props => {
  ...
  const onClickUpdateData = () => {
    fetch(selectedAuthor ? URL_RANDOM_BY_AUTHOR.replace(AUTHOR_TAG, selectedAuthor) : URL_RANDOM_QUOTE).then(response => response.json()).then(({
      author,
      quote
    }) => {
      // let author, content;
...
const QuoteAPIFrontendInspectorControls = props => {
  ...
  const {
    tags: initialTags
  } = getBlock(idInnerParagraph).attributes;
  ...
  const suggestionsTags = _constants_index_js__WEBPACK_IMPORTED_MODULE_6__.tags.map(tag => tag.name);
  return /*#__PURE__*/(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_7__.jsx)(_wordpress_block_editor__WEBPACK_IMPORTED_MODULE_3__.InspectorControls, {
...
function addAttributes(settings) {
  if ('core/quote' !== settings.name && 'core/paragraph' !== settings.name) {
    return settings;
  }
  let extraAttributes = {
    namespace: {
